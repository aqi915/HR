...
<script type="text/javascript">
	var doDelete = function(ygh) {
		if (confirm("确定要删除，小心手滑")) {
			location.href = "./userservlet?action=deleteuser&userid=" + ygh;
		}
	}

	var doAddGood = function() {
		location.href = "./addgood.jsp";
	}
</script>
<script type="text/javascript">
	function getTime() {
			...
			<table class="display stylized" id="example">
				...
				<tbody>
					...
					<c:forEach items="${users}" var="user">
						<tr>
							<td class="center">${user.ygh}</td>
							<td class="center">${user.xm}</td>
							<td class="center">${user.mm}</td>
							<c:if test="${user.qx==1}">
							...
							<td class="center"><a
								href="./userservlet?action=qxszch&userid=${user.ygh}">查看</a> <a
								href="./userservlet?action=qxszxg&userid=${user.ygh}">修改</a> <%-- <a
								href="javascript:doDelete(${user.ygh})">删除</a> --%></td>
						</tr>
					</c:forEach>
				</tbody>
				<tfoot>
